...
> **§4、§5 的 MB/s 帶有已知的高估**：當時的探針把**全部**收到的位元組除以「最後一則 − 第一則」
> 的時間，而那段時間只涵蓋 `n−1` 個到達間隔，所以數字比實際高 `n/(n−1)`——§4 的 921 KB 影像跑
> （25 樣本）約 **4%**，§5 那些數百 Hz、樣本數大的跑則可忽略。`tools/zping.py` 事後已修正
> （bytes 只累加第一則之後的訊息），**既有數據依「不覆寫」原則原封保留**，重跑時請注意新舊算法不同。
> 這個偏差不改變本文任何結論（判斷都建立在掉包率與相對比較上）。
